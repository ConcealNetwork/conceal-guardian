      ...
      CCXApi.info()
        .then((data) => {
          ...
          if (lastHeight !== data.height) {
            ...
            const duration = moment.duration(moment().diff(lastTS));

            if (duration.asSeconds() > (configOpts.restart.maxBlockTime || 1800)) {
              errorCallback(
                `No new block has be seen for more then ${(configOpts.restart.maxBlockTime || 1800) / 60} minutes`,
              );
              heightIsOK = false;
            }
          }
