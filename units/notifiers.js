...
function notifyViaDiscord(config, msgText, _msgType, nodeData) {
  if (oPath.get(config, "error.notify.discord.url", "")) {
    // Validate and sanitize data before sending
    const sanitizedNodeName = validator
      .escape(String(nodeData?.name || "Unknown"))
      .substring(0, 100);
    ...
    try {
      ...
      if (
        !allowedDiscordHostnames.includes(url.hostname) ||
    ...
    axios
      .post(discordUrl, discordPayload, {
        timeout: 10000,
        headers: {
...
function notifyViaEmail(config, msgText, _msgType, nodeData) {
  var auth = null;
  ...
  const sanitizedMsgText = validator.escape(String(msgText || "")).substring(0, 1000);
  const sanitizedSubject = validator
    .escape(
      String(oPath.get(config, "error.notify.email.message.subject", "Conceal Guardian Error")),
    )
    .substring(0, 200);
  const sanitizedFrom = validator
    .escape(String(oPath.get(config, "error.notify.email.message.from", "")))
    .substring(0, 200);
  const sanitizedTo = validator
    .escape(String(oPath.get(config, "error.notify.email.message.to", "")))
    .substring(0, 200);
