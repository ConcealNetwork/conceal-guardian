...
export function Initialize(configFileName) {
  ...
  var questions = [
    ...
    },
    {
      ...
      when: (answers) => answers.useFeeAddress,
    },
    {
      type: "confirm",
    ...
    },
    {
      ...
      name: "nodeUrl",
      message:
        "Will your node have a custom url? (if you do not know about it, just leave it empty)",
    ...
    },
    {
      ...
      when: (answers) => answers.notifyDiscord,
    },
    {
      type: "confirm",
      name: "notifyEmail",
    ...
    },
    {
      ...
      when: (answers) => answers.notifyEmail,
      validate: (value) => {
        if (value) {
          return true;
        } else {
    ...
    },
    {
      ...
      when: (answers) => answers.emailRequireAuth,
      validate: (value) => {
        if (value) {
          return true;
        } else {
  ...
  inquirer.prompt(questions).then((answers) => {
    ...
    answers.nodePath
      ? oPath.set(configOpts, "node.path", answers.nodePath)
      : oPath.del(configOpts, "node.path");
    answers.reachableOutside
      ? oPath.set(configOpts, "node.bindAddr", "0.0.0.0")
      : oPath.set(configOpts, "node.bindAddr", "127.0.0.1");
    answers.autoUpdate
      ? oPath.set(configOpts, "node.autoUpdate", true)
      : oPath.del(configOpts, "node.autoUpdate");

    answers.useFeeAddress
      ? oPath.set(configOpts, "node.feeAddr", answers.feeAddress)
      : oPath.del(configOpts, "node.feeAddr");
    answers.usePool
      ? oPath.set(configOpts, "pool.notify.url", answers.poolURL)
      : oPath.del(configOpts, "pool.notify");
    answers.notifyDiscord
      ? oPath.set(configOpts, "error.notify.discord.url", answers.discordHookURL)
      : oPath.del(configOpts, "error.notify.discord");
